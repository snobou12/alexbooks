...
const Pages = ({ selectedSize, pages }) => {
	...
	const handleSelectTemplate = (
		...
		dispatch(
			handleSetTemplateToPage({ sideToChange, tmplId, selectedSide, pageType })
		);
	};
	...
	function getTypeContent(typeId) {
		switch (typeId) {
			...
			case 1:
				return (
					<div
						className={`pages__type_content_color ${
							selectedSize === 0 && "pages__type_content_color--quadratic"
						}`}
					>
						<div className="pages__colors_content">
							<div className="pages__colors_leftside">
								<div className="pages__colors_title">Левая страница</div>
								...
								<div className="pages__colors">
									{pages.colors.map((color, idx) => (
										<div
											onClick={() => handleChangeColor(color.hex, "leftside")}
											style={{ backgroundColor: `#${color.hex}` }}
											key={`${color.id}:${idx}`}
											className="pages__colors_item"
										></div>
									))}
								</div>
							</div>
							<div className="pages__colors_righside">
								<div className="pages__colors_title">Правая страница</div>
								<div className="pages__colors_description">
									Выберите цвет страницы
								</div>
								<div className="pages__colors">
									{pages.colors.map((color, idx) => (
										<div
											onClick={() => handleChangeColor(color.hex, "rightside")}
											style={{ backgroundColor: `#${color.hex}` }}
											key={`${color.id}:${idx}`}
											className="pages__colors_item"
										></div>
		...
		<div className="cnsr__pages">
			...
			<div
				className={`cnsr__pages_rightside ${
					selectedSize === 1 &&
					pages.selectedType === 0 &&
					"cnsr__pages_rightside--landscape"
				}`}
			>
